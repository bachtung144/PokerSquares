...
public class PlayerEx implements PokerSquaresPlayer{
	...
	*/
	...
	public ArrayList<Integer> findCols(int row, Card card){
		...
		if(hasEmptyCol()){
			...
			for(int i = 0; i < size; i++){
				...
				/*
				if(colHasFourOfKind(i) && simGrid[row][i] == null){
					cols.clear();
					cols.add(i);
					return cols;
				}
	...
	public int returnPairCol(Card card){
		for(int i = 0; i < size; i++)
			if(hasPair(i, card))
				return i;
		return -1;
	...
	*/	
			...
			if(!fullRow(suitArray.indexOf(card.getSuit()))){
                ...
                if(canMakeChange(row, card)){
					...
					for(int i =0; i < rowsNoFlush.size(); i++){
						//if(canMakeChange(rowsNoFlush.get(i), card) && (changeT > 5 || numCardRow(suitArray.indexOf(-1)) < 2)){
	...
	public int[] getPlayMC(Card card, ArrayList<int[]> plays) {
		//System.out.println(card);
		//ArrayList<int[]>playsMC = new ArrayList<int[]>();
		/*
		Card[][] simGridNew = new Card[size][size];
		for(int i = 0; i < size; i++){
			for(int index = 0; index < size; index++){
				simGridNew[i][index] = simGrid[i][index];
			}
		}
		ArrayList<Integer> suitArrayMC = new ArrayList<Integer>();
		for(int i = 0; i < size; i++){
			suitArrayMC.add(suitArray.get(i));
		}
		*/
		if(plays.size() == 1){
			return plays.get(0);
	...
	public int[] playSim2(Card card){
		ArrayList<int[]> playsSim = new ArrayList<int[]>();
		int row;
		int col;
		if(boardIsEmpty){
			int[] playPos = {0, 0};
		...
		if(canCreateFullHouseUnlimited(card)){
			...
			int[] playPos = {row, col};
			boolean b = true;
			for(int[] play : playsSim){
				if(play[0] == playPos[0] && play[1] == playPos[1]){
					b = false;
				}
			}
			if(b == true){
				playsSim.add(playPos);
			}
		}
		if(!suitArray.contains(card.getSuit())){
			if(hasEmptyRow()){
			...
			if(!fullRow(suitArray.indexOf(card.getSuit()))){
                ...
                if(canMakeChange(row, card)){
					...
					ArrayList<Integer> rowsNoFlush = rowsNoFlush();
					for(int i =0; i < rowsNoFlush.size(); i++){
						if(canMakeChange(rowsNoFlush.get(i), card) && (changeT > 2 || numCardRow(suitArray.indexOf(-1)) < 2)){
						//if(canMakeChange(rowsNoFlush.get(i), card)){
							//System.out.println("same suit special: " + "changeT = " + changeT);
							col = tempCol;
							int[] playPos = {rowsNoFlush.get(i), col};
		...
		Random random = new Random();
		int[] play = playsSim.get(random.nextInt(playsSim.size()));
		simGrid[play[0]][play[1]] = card;
		if(isEmptyRow(play[0]) && !suitArray.contains(card.getSuit())){
			suitArray.set(play[0], card.getSuit());
		}
	...
	public void test(){
		PokerSquares.printGrid(simGrid);
	}

	public static void main(String[] args) {
		//new MyPlayer().test();
		
		new PokerSquares(new PlayerEx(), 60000L).play(); // play a single game
	}
}

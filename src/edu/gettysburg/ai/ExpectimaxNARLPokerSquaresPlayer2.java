...
public class ExpectimaxNARLPokerSquaresPlayer2 extends NARLPokerSquaresPlayer implements PokerSquaresPlayer  {
	...
	int nodeLimit = 1, depthLimit = 1;


	@SuppressWarnings("unchecked")
	public ExpectimaxNARLPokerSquaresPlayer2(int nodeLimit) {
	...
	public Position getExpectimaxPlay(Card card, int depth) { // depth-0 choice node
		// compute depth limit
		int i = numPlays;
		int nodeTotal = (SIZE * SIZE - i) ;
		depthLimit = -1;
		while (nodeTotal <= nodeLimit && i < SIZE * SIZE) {
			depthLimit++;
			i++;
			nodeTotal *= (NUM_CARDS - i) * (SIZE * SIZE - i); // * number of cards * number of empty positions
		}
...
//		System.out.println("Depth Limit: " + depthLimit);
	...
	public static void main(String[] args) {
		ExpectimaxNARLPokerSquaresPlayer2 player = new ExpectimaxNARLPokerSquaresPlayer2(100000);
